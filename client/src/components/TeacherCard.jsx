import { User, Briefcase, GraduationCap, MapPin, Image } from 'lucide-react';

const COURSE_COLORS = {
  art: 'bg-primary/10 text-primary',
  dance: 'bg-secondary/20 text-yellow-700',
};

const TeacherCard = ({ teacher, onViewArtwork }) => {
  const hasArtwork = teacher.artPhotos && teacher.artPhotos.length > 0;
    ...
    <div className="card overflow-hidden group text-center">
      <div className="relative">
        <div className="h-52 bg-accent flex items-center justify-center overflow-hidden">
          {teacher.photo ? (
            <img src={resolveUrl(teacher.photo)} alt={teacher.name}
      ...
      <div className="p-5">
        <h3 className="font-display text-lg font-semibold text-dark mb-3 group-hover:text-primary transition-colors">{teacher.name}</h3>

        <div className="flex items-center gap-2 mb-3">
          <span className={`flex-1 text-xs font-semibold px-3 py-2 rounded-lg font-body capitalize text-center ${
            COURSE_COLORS[teacher.courseType] || 'bg-gray-100 text-gray-600'
          }`}>{teacher.courseType}</span>
          <button onClick={() => hasArtwork && onViewArtwork(teacher)} disabled={!hasArtwork}
            ...
            <Image size={13} />
            {hasArtwork ? `Artwork (${teacher.artPhotos.length})` : 'No Artwork'}
          </button>
        </div>

        {teacher.qualification && (
          <div className="flex items-center justify-center gap-1.5 mt-2 text-sm text-gray-500 font-body">
        ...
        {teacher.skills && teacher.skills.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1 justify-center">
            {teacher.skills.slice(0, 3).map((skill, i) => (
              <span key={i} className="text-xs bg-accent text-gray-600 px-2 py-0.5 rounded-full font-body">{skill}</span>
            ))}
            {teacher.skills.length > 3 && (
              <span className="text-xs bg-accent text-gray-500 px-2 py-0.5 rounded-full font-body">+{teacher.skills.length - 3}</span>
            )}
          </div>
        )}
      </div>
