...
import {
  ArrowLeft, Trash2, User, Camera, Pencil, Save, X,
} from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_URL?.replace('/api', '') || '';

const COURSE_TYPES = ['art', 'dance', 'music', 'yoga'];

/* ── Info card — matches student profile style ─────────────────── */
...
const AdminTeacherProfile = () => {
  ...
  const handleArtUpload = async (e) => {
    ...
    try {
      await api.post(`/teachers/${id}/art`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
          ...
          <div className="bg-primary px-7 py-6 flex items-center gap-5 relative">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-white/60 font-body mt-0.5 capitalize">
                ...
                {(editing ? form.qualification : teacher.qualification) && (
                  <> · {editing ? form.qualification : teacher.qualification}</>
                )}
              </p>
            </div>
              ...
              <>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skillsList.length > 0 ? skillsList.map(s => (
                    <span key={s} className="text-xs bg-white border border-gray-200 text-gray-600 px-2.5 py-1 rounded-full font-body">
                      {s}
                    </span>
                  )) : (
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm font-body">
                ...
                <div className="flex flex-col gap-1.5 sm:col-span-2 lg:col-span-3">
                  ...
                  <input
                    ...
                    value={form.skills}
                    onChange={handleFormChange}
                    placeholder="e.g. Painting, Sculpture, Charcoal"
                    className="input-field py-2 text-sm"
                  />
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-gray-400 font-body mt-0.5">Photos uploaded by or for this teacher</p>
            </div>
              ...
              {teacher.artPhotos.map((photo, i) => (
                <div key={i} className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 border border-gray-100">
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-3 py-2
                    ...
                    <p className="text-gray-300 text-[10px] font-body truncate capitalize">
                      {[teacher.courseType, teacher.qualification].filter(Boolean).join(' · ')}
                    </p>
                  </div>
